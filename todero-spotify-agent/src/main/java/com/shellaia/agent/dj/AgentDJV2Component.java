...
import com.shellaia.agent.dj.loop.AgentDecisionLoop;
...
@AIAController(name = "com.shellaia.agent.dj.v2",
    type = ServerType.AI,
    visible = true,
...
public class AgentDJV2Component {
  ...
  public Boolean capabilities(CommandContext context) {
    try {
      AgentCapabilityManifest manifest = ToolAgentCapabilitySupport.resolveManifest(
          storage,
          context,
          new DjAgentV2Capabilities().manifest(),
          SPOTIFY_COMPONENT,
  ...
  private static void writeChannels(com.fasterxml.jackson.databind.node.ObjectNode projections,
    ...
    com.fasterxml.jackson.databind.node.ObjectNode projectionWebview = projections.putObject("html");
    if (html.isEmpty()) {
    ...
    com.fasterxml.jackson.databind.node.ObjectNode webview = channelsNode.putObject("html");
    if (html.isEmpty()) {
